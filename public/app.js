...
function displayStorybook(storybook) {
    ...
    // UIHelper로 헤더 렌더링
    let html = UIHelper.renderStorybookHeader(storybook);
...
async function generateIllustration(pageIndex) {
    ...
    const characterReferences = currentStorybook.characters
        .filter(char => char.referenceImage)
        .map(char => ({ name: char.name, description: char.description, referenceImage: char.referenceImage }));
    ...
    // 3. 로딩 UI 표시
    showLoadingUI(illustrationDiv, 'AI가 삽화를 생성하는 중...');
...
function downloadAllText() {
    ...
    if (currentStorybook.translations && typeof currentStorybook.translations === 'object') {
        availableLanguages.push(...Object.keys(currentStorybook.translations));
    }
    
    console.log(`📥 언어별 텍스트 다운로드: ${availableLanguages.join(', ')}`);
    ...
    availableLanguages.forEach(lang => {
        ...
        // 학습 단어 섹션 추가 (한국어 파일에만)
        if (lang === 'ko' && 
            currentStorybook.educational_content && 
        ...
        const a = document.createElement('a');
        a.href = url;
        a.download = `${currentStorybook.title}_${langName}_페이지텍스트.txt`;
        document.body.appendChild(a);
        a.click();
        document.body.removeChild(a);
    ...
    showNotification('success', '다운로드 완료', `${availableLanguages.length}개 언어의 텍스트 파일이 다운로드되었습니다.`);
}
